class B77HS_CDU_MainDisplay extends FMCMainDisplay {
    ...
    setupAimsTriggers() {
        Coherent.on('B77HS_AIMS_SEND_MESSAGE_TO_MCDU', (message) => {
        ...
        Coherent.on('B77HS_AIMS_RECALL_MESSAGE_FROM_MCDU_WITH_ID', (text) => {
   ...
   /*  mcduServerClientEventHandler(event) {
        switch (event.type) {
            case 'open': {
                ...
                (new NXNotifManager).showNotification({title: "MCDU CONNECTED",
                    message: "B77HS MCDU successfully connected to SimBridge MCDU Server.", timeout: 5000});
            ...
            case 'message': {
                ...
                if (messageType === 'event') {
                    ...
                    SimVar.SetSimVarValue(`L:B77HS_MCDU_PUSH_ANIM_${mcduIndex}_${button}`, "Number", 1);
                }
    ...
    Init() {
        ...
        SimVar.SetSimVarValue("L:B77HS_GPS_PRIMARY_LOST_MSG", "Bool", 0).then();
    ...
    onUpdate(_deltaTime) {
        ...
        if (this.powerCheckUpdateThrottler.canUpdate(_deltaTime) !== -1
            ...
            const isPoweredL = SimVar.GetSimVarValue("L:B77HS_ELEC_AC_ESS_SHED_BUS_IS_POWERED", "Number");
    ...
    checkAocTimes() {
        if (!this.aocTimes.off) {
            if (this.flightPhaseManager.phase === AimsFlightPhases.TAKEOFF && !this.isOnGround()) {
                // Wheels off
        ...
        if (!this.aocTimes.out) {
            const currentPKGBrakeState = SimVar.GetSimVarValue("L:B77HS_PARK_BRAKE_LEVER_POS", "Bool");
            if (this.flightPhaseManager.phase === AimsFlightPhases.PREFLIGHT && !currentPKGBrakeState) {
    ...
    setTemplate(template, large = false) {
        ...
        SimVar.SetSimVarValue("L:AIRLINER_CDU_CURRENT_FPLN_WAYPOINT", "number", this.currentFlightPlanWaypointIndex).then();
    ...
    initKeyboardScratchpad() {
        window.document.addEventListener('click', () => {
            ...
            const cduInput = NXDataStore.get("CDU_KB_INPUT", "DISABLED");
            const cduTimeout = parseInt(NXDataStore.get("CONFIG_MCDU_KB_TIMEOUT", "60"));
            const isPoweredL = SimVar.GetSimVarValue("L:B77HS_ELEC_AC_ESS_SHED_BUS_IS_POWERED", "Number");
            const isPoweredR = SimVar.GetSimVarValue("L:B77HS_ELEC_AC_2_BUS_IS_POWERED", "Number");
                ...
                if (this.inFocus && (isPoweredL || isPoweredR)) {
                    ...
                    if (cduTimeout) {
                        this.check_focus = setInterval(() => {
                            if (Math.abs(new Date() - this.lastInput) / 1000 >= cduTimeout) {
                                this.clearFocus();
                            }
                        }, Math.min(cduTimeout * 1000 / 2, 1000));
                    }
                } else {
                    this.clearFocus();
                }
        ...
        window.document.addEventListener('keydown', (e) => {
            ...
            if (this.inFocus && SimVar.GetSimVarValue("L:B77HS_ELEC_AC_ESS_SHED_BUS_IS_POWERED", "Number")) {
                ...
                if (e.altKey || (e.ctrlKey && keycode === KeyCode.KEY_Z)) {
                    ...
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_1_CLR", "Number", 1);
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_2_CLR", "Number", 1);
                    ...
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_1_" + letter.toUpperCase(), "Number", 1); // TODO: L/R [1/2] side MCDU Split
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_2_" + letter.toUpperCase(), "Number", 1);
                } else if (keycode === KeyCode.KEY_PERIOD || keycode === KeyCode.KEY_DECIMAL) {
                    this.onDot();
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_1_DOT", "Number", 1);
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_2_DOT", "Number", 1);
                } else if (keycode === KeyCode.KEY_SLASH || keycode === KeyCode.KEY_BACK_SLASH || keycode === KeyCode.KEY_DIVIDE || keycode === 226) {
                    this.onDiv();
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_1_SLASH", "Number", 1);
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_2_SLASH", "Number", 1);
                    ...
                    this.onSp();
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_1_SP", "Number", 1);
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_2_SP", "Number", 1);
                } else if (keycode === 189 || keycode === KeyCode.KEY_SUBTRACT) {
                    this.onPlusMinus("-");
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_1_PLUSMINUS", "Number", 1);
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_2_PLUSMINUS", "Number", 1);
                    ...
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_1_L" + (func_num + 1), "Number", 1);
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_2_L" + (func_num + 1), "Number", 1);
                } else if (keycode >= KeyCode.KEY_F7 && keycode <= KeyCode.KEY_F12) {
                    const func_num = keycode - KeyCode.KEY_F7;
                    this.onRightFunction(func_num);
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_1_R" + (func_num + 1), "Number", 1);
                    SimVar.SetSimVarValue("L:B77HS_MCDU_PUSH_ANIM_2_R" + (func_num + 1), "Number", 1);
                }
            }
        });
    ...
    onEvent(_event) {
        ...
        // MCDU should not accept input while unpowered
        if (!SimVar.GetSimVarValue("L:B77HS_ELEC_AC_ESS_SHED_BUS_IS_POWERED", "Number")) {
            return;
        }
    ...
    printPage(lines) {
        ...
        if (SimVar.GetSimVarValue("L:B77HS_PRINTER_PRINTING", "bool") === 1) {
            SimVar.SetSimVarValue("L:B77HS_PAGES_PRINTED", "number", SimVar.GetSimVarValue("L:B77HS_PAGES_PRINTED", "number") + 1);
            SimVar.SetSimVarValue("L:B77HS_PRINT_PAGE_OFFSET", "number", 0);
        }
        SimVar.SetSimVarValue("L:B77HS_PRINT_LINES", "number", lines.length);
        SimVar.SetSimVarValue("L:B77HS_PAGE_ID", "number", SimVar.GetSimVarValue("L:B77HS_PAGE_ID", "number") + 1);
        SimVar.SetSimVarValue("L:B77HS_PRINTER_PRINTING", "bool", 0).then(v => {
            this.aimsMesssagesListener.triggerToAllSubscribers('B77HS_PRINT', formattedValues);
            //this.sendToMcduServerClient(`print:${JSON.stringify({lines: websocketLines})}`);
            setTimeout(() => {
                SimVar.SetSimVarValue("L:B77HS_PRINTER_PRINTING", "bool", 1);
                this.printing = false;
            }, 2500);
        });
    }
    ...
    sendUpdate() {
        ...
        let right = left;
        if (SimVar.GetSimVarValue("L:B77HS_ELEC_AC_ESS_SHED_BUS_IS_POWERED", "bool")) {
            left = {
                lines: [
                    this._labels[0],
                    this._lines[0],
        ...
        if (SimVar.GetSimVarValue("L:B77HS_ELEC_AC_2_BUS_IS_POWERED", "bool")) {
            right = left;
        }
