...
class FMCDataManager {
    constructor(_fmc) {
        ...
        if (stored !== null) {
            JSON.parse(stored).forEach((wp) => {
                if (wp) {
                    this.storedWaypoints.push(Aims.WaypointBuilder.fromCoordinates(wp.ident, new LatLongAlt(wp.coordinates), this.fmc, wp.additionalData));
                } else {
    ...
    _createWaypoint(ident, index, coordinates, additionalData, stored) {
        ...
        const wp = Aims.WaypointBuilder.fromCoordinates(ident, coordinates, this.fmc, additionalData);
    ...
    createPlaceBearingPlaceBearingWaypoint(place1, bearing1, place2, bearing2, stored = false, ident = undefined) {
        const coordinates = B77HS_Util.greatCircleIntersection(place1.infos.coordinates, bearing1, place2.infos.coordinates, bearing2);
    ...
    createRunwayWaypoint(airport, runway) {
        ...
        const wp = Aims.WaypointBuilder.fromCoordinates(ident, runway.beginningCoordinates, this.fmc.instrument);
...
FMCDataManager.STORED_WP_KEY = 'B77HS.StoredWaypoints';