...
class NXSpeedsUtils {
    ...
    static getVtargetGSMini(vapp, windDiff) {
        return Math.max(vapp, Math.min(Math.round(vapp + windDiff), Math.round(
            SimVar.GetSimVarValue("L:B77HS_FLAPS_HANDLE_INDEX", "Number") === 4 ? SimVar.GetSimVarValue("L:B77HS_SPEEDS_VMAX", "Number") - 5 : SimVar.GetSimVarValue("L:B77HS_SPEEDS_VFEN", "Number")
        )));
    }
