 ...
 */
B77HS_Util.trueToMagnetic = (heading, magVar) => {
 ...
 */
B77HS_Util.magneticToTrue = (heading, magVar) => {
    return (720 + heading + (magVar || SimVar.GetSimVarValue("MAGVAR", "degree"))) % 360;
};
 ...
 * @returns {LatLong}
 */
B77HS_Util.sphericalToLatLon = (s) => {
 ...
 */
B77HS_Util.getIsaTemp = (alt = Simplane.getAltitude()) => {
 ...
 */
B77HS_Util.getIsaTempDeviation = (alt = Simplane.getAltitude(), sat = Simplane.getAmbientTemperature()) => {
    return sat - B77HS_Util.getIsaTemp(alt);
};
...
B77HS_Util.meterToFeet = (meterValue) => {
