...
export function isOnGround(): boolean {
    return SimVar.GetSimVarValue('L:B77HS_LGCIU_1_NOSE_GEAR_COMPRESSED', 'bool')
        || SimVar.GetSimVarValue('L:B77HS_LGCIU_2_NOSE_GEAR_COMPRESSED', 'bool');
}
...
function isEngineOnTakeOffThrust(index: number): boolean {
    return SimVar.GetSimVarValue(`L:B77HS_ENGINE_N1:${index}`, 'number') >= 70;
}
