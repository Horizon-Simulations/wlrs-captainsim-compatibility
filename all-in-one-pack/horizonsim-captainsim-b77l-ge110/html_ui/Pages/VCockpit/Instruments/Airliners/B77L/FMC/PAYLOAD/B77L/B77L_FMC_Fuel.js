class FMC_Fuel {
    static ShowPage(fmc) {
        ...
        const currentFuel = (SimVar.GetSimVarValue("FUEL TOTAL QUANTITY", "gallons") * weightPerGallon).toFixed(0);
        const targetFuel = kgToUser(SimVar.GetSimVarValue("L:B777_FUEL_REQUEST", "KG"));         

        const targetFuelText = targetFuel <= 10 ? "□□□□□□" : `${kgToUser(targetFuel).toFixed(0)}{small}${storedUnits}`;
        ...
        fmc.onRightInput[0] = () => {
            ...
            if (value) {
                ...
                if (value >= 0 && userToKg(value) <= SimVar.GetSimVarValue("FUEL WEIGHT PER GALLON", KG) * 47890) { //improve later
...
async function setDesiredFuel(fmc, updateView, blockFuel) {
    ...
    const rightMainTankCapacity = 9560;
