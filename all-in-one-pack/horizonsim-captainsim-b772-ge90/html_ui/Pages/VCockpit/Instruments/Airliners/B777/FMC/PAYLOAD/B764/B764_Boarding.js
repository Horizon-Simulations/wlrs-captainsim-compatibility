class HorizonSimBoarding {
    ...
    static async setTargetPax(numberOfPax) {
        ...
        await fillStation(paxStations["businessClass"], 0.14, numberOfPax);
        await fillStation(paxStations["firstClass"], 0.03, paxRemaining);
    }

    static async setTargetCargo(cargo) {
        SimVar.SetSimVarValue("L:B777_DESIRED_CARGO", "number", SaltyUnits.kgToUser(cargo));

        const maxLoadInCargoHold = 67034;   //43900; // from flight_model.cfg ???      //calibrate later
    ...
    async update(_deltaTime) {
        ...
        const boardingRate = WTDataStore.get("747_CONFIG_BOARDING_RATE", "REAL");
