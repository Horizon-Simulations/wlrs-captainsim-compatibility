class WT_VerticalAutopilot {
    ...
    get navMode() {
        const navMode = this._navModeSelector.lNavModeState;    //const navSource = 3;
    ...
    update() {
        ...
        //to force all of the other mode to stop
        ...
        switch (this._vnavPathStatus) {
            ...
            case VnavPathStatus.PATH_ARMED:
                ...
                if (this.canPathActivate()) {
                    ...
                    SimVar.SetSimVarValue("L:AP_FLCH_ACTIVE", "number", 0); //fix bug
        ...
        /* reactivated, seems like this recalc the descend distance*/
        ...
        switch (this.isVNAVOn) {
            ...
            case true:
                ...
                if (this.path.fpa !== undefined && this.path.fpa == 0 && this.nextPath.fpa !== undefined && this.nextPath.fpa != 0) {
                    if (this.nextPath.deviation <= 1000 && this.selectedAltitude < this.indicatedAltitude - 100) {                  //return true;
                        return true;
                    }
                }
                if (this.firstPossibleDescentIndex && this._vnav.flightplan.activeWaypointIndex >= this.firstPossibleDescentIndex) {
                    ...
                    if (this.path.deviation <= 1000 && altitudeDifference > 100 && this.distanceToTod < 20
                        && this.selectedAltitude < this.indicatedAltitude - 100 && (this.selectedAltitude < this.lockedAltitude - 100 || this.lockedAltitude === undefined)) {                      //return true;
                        return true;
    ...
    canPathActivate(alreadyActive = false) {
        if (!alreadyActive) {
            ...
            if (this.path.fpa != 0 && this.path.deviation < 1000 && this.path.deviation > gsBasedDeviation) {   //-350
                return true;
            }
            return false;
        } else {
            if (this.path.fpa != 0 && this.path.deviation < 1000 && this.path.deviation > -1000) {  //-350
                return true;
            }
            return false;
        }
    ...
    canGlideslopeActivate() {
		const gsi = SimVar.GetSimVarValue('NAV GSI:' + this.navMode, 'number');        //const gsi = SimVar.GetSimVarValue("NAV GSI:3", "number");
    ...
    checkGlideslopeStatus(activeNavIndex = false) {
        let navIndex = this.navMode == 0 ? 1 : this.navMode;            //let navIndex = 3;
		if (activeNavIndex) {
    ...
    trackGlideslope() {
        const gsi = SimVar.GetSimVarValue('NAV GSI:' + this.navMode, 'number'); //const gsi = SimVar.GetSimVarValue("NAV GSI:3", "number");
        const gslla = SimVar.GetSimVarValue('NAV GS LATLONALT:' + this.navMode, 'latlonalt');   //const gslla = SimVar.GetSimVarValue("NAV GS LATLONALT:3", "latlonalt");
        ...
        switch (this._constraintStatus) {
            case ConstraintStatus.NONE:
                if (this.constraint.index !== undefined && this.constraint.index >= this._vnav.flightplan.activeWaypointIndex) {
                    this.setConstraintAltitude(true);       //used to be no param
                }
                break;
            case ConstraintStatus.OBSERVING_CLIMB:
                ...
                Coherent.call('GENERAL_ENG_THROTTLE_MANAGED_MODE_SET', ThrottleMode.TOGA);  //used to be nothing
                break;
    ...
    //PAUSE HEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEEHEREEEHREEEE
    /**
    ...
    setSnowflake(value = undefined) {
        ...
        if (isGlidepathActive || isPathActive) {
            ...
            } else if (this.path.deviation && Math.abs(this.path.deviation) < 1000) {                   //} else if (this.path.deviation) {
    ...
    captureAltitude(altitude) {
        ...
        if (dt > 250) {
            ...
            setVerticalSpeed = 100 * Math.ceil(setVerticalSpeed / 100) * 10;         //setVerticalSpeed = 100 * Math.ceil(setVerticalSpeed / 100);
            this.vsSlot2Value = setVerticalSpeed;
            if (Math.abs(deltaAlt) < 20) {
                ...
                switch (this.verticalMode) {
                    case VerticalNavModeState.ALTCAP:
                        ...
                        SimVar.SetSimVarValue("L:AP_ALT_HOLD_ACTIVE", "number", 1);     //ROBSIM TEMPLATE COMPATIBLITY
                        break;
                    case VerticalNavModeState.ALTSCAP:
        ...
        //this._navModeSelector.activateSpeedMode(); //DEACTIVATE
    }
