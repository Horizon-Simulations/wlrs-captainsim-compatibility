...
const getSimBriefPlan = (fmc, store, updateView) => {
    const userid = WTDataStore.get("OPTIONS_SIMBRIEF_ID", "");
    const username = WTDataStore.get("OPTIONS_SIMBRIEF_USER", "");
    if (!userid && !username) {
    ...
    const handleData = (data) => {
        fmc.simbrief["aircraftICAO"] = data.aircraft.icaocode;
        fmc.simbrief["aircraftBaseType"] = data.aircraft.base_type;
        ...
        fmc.simbrief["originPlanRwy"] = data.origin.plan_rwy;
        fmc.simbrief["destinationPlanRwy"] = data.destination.plan_rwy;
        ...
        fmc.simbrief["alternateIcao"] = data.alternate.icao_code; // under work, this can be multiple alternates
        fmc.simbrief["alternatePlanRwy"] = data.alternate.plan_rwy;
        ...
        /* PASSENGER */
        fmc.simbrief["paxCount"] = data.general.passengers;
        /* WEIGHT */
        fmc.simbrief["payload"] = data.weights.payload;
        fmc.simbrief["cargoLoad"] = data.weights.cargo;

        /* FIND THE TAKEOFF PLAN RUNWAY */
        fmc.simbrief["takeOffPlanRunway"] = findRunway(data, fmc.simbrief["originPlanRwy"]);
    ...
    // Define the findRunway function
    function findRunway(data, plannedRunway) {
        for (let i = 0; i < data.tlr.takeoff.runway.length; i++) {
            if (data.tlr.takeoff.runway[i].identifier === plannedRunway) {
                return data.tlr.takeoff.runway[i];
            }
        }
        return null;
    }
...
const getFplnFromSimBrief = async (fmc) => {
    this.url = "http://www.simbrief.com/api/xml.fetcher.php?json=1&username=" + WTDataStore.get("OPTIONS_SIMBRIEF_USER", "");
    const userID = WTDataStore.get("OPTIONS_SIMBRIEF_ID", "");
    if (userID != "") {
        this.url = "http://www.simbrief.com/api/xml.fetcher.php?json=1&userid=" + WTDataStore.get("OPTIONS_SIMBRIEF_ID", "");
    }    
