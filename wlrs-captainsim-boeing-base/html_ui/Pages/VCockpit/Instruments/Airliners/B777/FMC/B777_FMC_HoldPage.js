...
class B777_FMC_HoldsPage {
  ...
  constructor(fmc) {
    ...
    this._state = {
      ...
      fromWaypointIndex: 0,
      isModifying: false,
      page: B777_FMC_HoldsPage.FPLN_HOLD
    };
  }
  ...
  update(forceUpdate = false) {
    ...
    const currentHolds = B777_FMC_HoldsPage.getFlightplanHolds(this._fmc);

    if (currentHolds.length === 0) {
      B777_FMC_LegsPage.ShowPage1(this._fmc, true);
    }
    else {
      if (this._state.page === B777_FMC_HoldsPage.HOLD_LIST) {
        if (currentHolds.length > 1) {
          this.bindHoldListInputs(currentHolds);
          this.renderHoldList(currentHolds);
        }
        else {
          this._state.page = B777_FMC_HoldsPage.FPLN_HOLD;
        }
      }
  
      if (this._state.page === B777_FMC_HoldsPage.FPLN_HOLD) {
  ...
  bindFplnHoldInputs(currentHold, numHolds) { 
    ...
    if (numHolds < 6 && !this._state.isModifying) {
      this._fmc.onLeftInput[5] = () => B777_FMC_LegsPage.ShowPage1(this._fmc, true);
    }
  ...
  bindHoldListInputs(currentHolds) {
    for (let i = 0; i < 6; i++) {
      if (i < 5 && currentHolds[i] !== undefined) {
        this._fmc.onLeftInput[i] = () => B777_FMC_HoldsPage.showHoldPage(this._fmc, currentHolds[i].waypoint.ident);
      }
      else if (i >= 5 && currentHolds[i] !== undefined) {
        this._fmc.onRightInput[i - 5] = () => B777_FMC_HoldsPage.showHoldPage(this._fmc, currentHolds[i].waypoint.ident);
      }
    }
    ...
    if (currentHolds.length < 6) {
      this._fmc.onRightInput[5] = () => B777_FMC_LegsPage.ShowPage1(this._fmc, true);
    }
  }
  ...
  calculateETA(currentHold) {
    ...
    if (activeHoldIndex !== -1 && activeHoldIndex === currentHold.index) {
      const eteSeconds = Math.round(SimVar.GetSimVarValue("L:WT_B777_WPT_DISTANCE", "number") / (groundSpeed / 3600));
      ...
      const planePosition = B777_FMC_HoldsPage.getPlanePosition();
  ...
  static showHoldPage(fmc, ident) {
    const instance = new B777_FMC_HoldsPage(fmc);
    const holds = B777_FMC_HoldsPage.getFlightplanHolds(fmc);

    const holdPageIndex = holds.findIndex(x => x.waypoint.ident === ident);
    if (holdPageIndex !== -1) {
      instance._state.pageNumber = holdPageIndex + 1;
      instance._state.page = B777_FMC_HoldsPage.FPLN_HOLD;
    }

    instance._state.isModifying = fmc._fpHasChanged;
    B777_FMC_HoldsPage.Instance = instance;
  ...
  static showHoldList(fmc) {
    ...
    instance._state.page = B777_FMC_HoldsPage.HOLD_LIST;
    instance._state.isModifying = fmc._fpHasChanged;
    B777_FMC_HoldsPage.Instance = instance;
  ...
  static handleHoldPressed(fmc) {
    ...
    if (holds.length === 0) {
      ...
      B777_FMC_HoldsPage.showHoldPage(fmc);
    }
    else if (holds.length > 1) {
      B777_FMC_HoldsPage.showHoldList(fmc);
    }
  }
}

B777_FMC_HoldsPage.Instance = undefined;

B777_FMC_HoldsPage.FPLN_HOLD = 'FPLN HOLD';
B777_FMC_HoldsPage.HOLD_LIST = 'HOLD LIST';

B777_FMC_HoldsPage.NONE = 'NONE';
B777_FMC_HoldsPage.ADD = 'ADD';
B777_FMC_HoldsPage.CHANGE_EXISTING= 'CHANGE_EXISTING';