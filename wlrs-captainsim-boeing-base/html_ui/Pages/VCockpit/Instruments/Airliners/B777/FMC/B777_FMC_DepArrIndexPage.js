class B777_FMC_DepArrIndexPage {
    static ShowPage1(fmc) {
        if (fmc.previousInput === "DEPARR") {
            B777_FMC_DepArrIndexPage.ShowIndexPage(fmc);
            return;
        }
        ...
        let directEntryPage = B777_FMC_DepArrIndexPage.ShowIndexPage;
        if (origin) {
            if (!airborne) {
                directEntryPage = B777_FMC_DepArrIndexPage.ShowDeparturePage;
            }
                ...
                if (distanceToOrigin > 50 || aircraftMovedMoreThanHalfwayToDestination) {
                    directEntryPage = B777_FMC_DepArrIndexPage.ShowArrivalPage;
                }
            }
        }
        directEntryPage(fmc);
    ...
    static ShowIndexPage(fmc) {
        ...
        if (origin) {
            ...
            fmc.onLeftInput[0] = () => {
                B777_FMC_DepArrIndexPage.ShowDeparturePage(fmc);
            };
        }
        let rowDestination = [""];
        let destination = fmc.flightPlanManager.getDestination();
        if (destination) {
            ...
            fmc.onRightInput[1] = () => {
                B777_FMC_DepArrIndexPage.ShowArrivalPage(fmc);
            };
        }
        fmc.setTemplate([
            ["DEP/ARR INDEX"],
    ...
    static ShowDeparturePage(fmc, currentPage = 1) {
            ...
            fmc.onRightInput[0] = () => {
                ...
                fmc.setRunwayIndex(-1, () => {
                    fmc.setDepartureIndex(-1, () => {
                        ...
                        B777_FMC_DepArrIndexPage.ShowDeparturePage(fmc, currentPage);
                    });
                });
            };
        }
                ...
                fmc.onRightInput[i] = () => {
                    if (fmc.flightPlanManager.getDepartureProcIndex() === -1) {
                        fmc.setOriginRunwayIndex(runwayIndex, () => {
                            B777_FMC_DepArrIndexPage.ShowDeparturePage(fmc, undefined);
                        });
                    }
                    else {
            ...
            for (let i = 0; i < departurePages[displayedPageIndex].length; i++) {
                ...
                fmc.onLeftInput[i] = () => {
                    ...
                    fmc.setDepartureIndex(departureIndex, () => {
                        fmc.flightPlanManager.resumeSync();
                        //fmc.flightPlanManager.setDepartureEnRouteTransitionIndex(-1, () => {B777_FMC_DepArrIndexPage.ShowDeparturePage(fmc);});
                        B777_FMC_DepArrIndexPage.ShowDeparturePage(fmc);
                    });
                };
            }
        ...
        fmc.onPrevPage = () => {
            if (currentPage > 1) {
                B777_FMC_DepArrIndexPage.ShowDeparturePage(fmc, currentPage - 1);
            } else {
                B777_FMC_DepArrIndexPage.ShowDeparturePage(fmc, pageCount);
            }
        };
        fmc.onNextPage = () => {
    ...
    static ShowArrivalPage(fmc, currentPage = 1) {
        ...
        if (selectedApproach) {
            ...
            fmc.onRightInput[0] = () => {
                ...
                fmc.setApproachIndex(-1, () => {
                    fmc.flightPlanManager.resumeSync();
                    B777_FMC_DepArrIndexPage.ShowArrivalPage(fmc, currentPage);
                });
            };
            rows[1] = ["", "TRANS"];
        ...
        fmc.onExecPage = () => {
            ...
            B777_FMC_DepArrIndexPage.ShowArrivalPage(fmc);
            fmc.refreshPageCallback = () => B777_FMC_DepArrIndexPage.ShowArrivalPage(fmc);
        };
        //end of CWB EXEC handling
        modStr = fmc.fpHasChanged ? "MOD" : "ACT";
        ...
        fmc.onPrevPage = () => {
            if (currentPage > 1) {
                B777_FMC_DepArrIndexPage.ShowArrivalPage(fmc, currentPage - 1);
            } else {
                B777_FMC_DepArrIndexPage.ShowArrivalPage(fmc, pageCount);
            }
        };
        fmc.onNextPage = () => {
            if (currentPage < pageCount) {
